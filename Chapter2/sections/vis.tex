...
\paragraph{Application Examples}
Exploring a large image collection, such as the set of images available on the Internet, is challenging. Besides the low-level visual features, the semantic contents of images are also effective in searching for relevant ones. Image classification techniques can be used to extract such semantic contents. For example, Fan~et~al.~\cite{Fan2004} detect salient objects in images and associate them with predefined semantic contents according to their perceptual properties. Similar contents are then grouped into a higher level semantic concept; for instance, ``sand field'', ``sea water'' and ``boat'' salient objects construct the concept of ``sea world''. Visualization can help make the output of classification algorithms more interpretable and interactive. To provide an overview of an image collection, Yang~et~al.~\cite{Yang2006} shows the extracted semantic contents, with each as a glyph, in a 2D display so that related contents are located close together using a multidimensional dimension scaling method~\cite{Borg2005}. Similarly, images are also displayed based on their similarity as in Figure 1. Zooming and panning are provided to make the visualization more scalable. When an image is selected, the visualization can be switched to a \emph{rainfall} mode, in which the selected image is shown at the bottom and related images are stacked atop based on their similarity with the selected one (figure b). Users are allowed to reassign the contents computed for each image; however, the model does not take into account the changes to improve its accuracy when classifying new images.

\cite{Migut2010} problem: considering a binary classifiers: two types of errors: false positives and false negatives. cost between these are different. visualize the model with decision boundary nad performance curve. explain that curve. changing 'opearing point' to adjust FPr or FNr. Also reassign class.

\cite{Bosch2013} classification needs training data, which is time-consuming and laborious if doing manually => interactive visual classifers"- first, multiple keyword coocurrence filters to have a set of core relevant tweets
- then, use them as positive messages and irrelevant as negative messages in a SVM classifier
- user inspects and corrects, changes update classifier
- also iterations of self-training
- classifier used for live tweets"	- tweets with geotagged shown as dots on the map, color coded by relevant/irrelevant interest and brightness based on classification confidence

\cite{Krause2014} An essential step in classification of high dimensional datasets is feature selection. The data may contain redundant or irrlevant features, thus can be removed without much loss of information. This step simplifies the model and reduces training time. XXX provide a visualization tool 
...
--> the problem is too many features for a complex process"	"- contribution is to improve the classification result
"	"- glyph-based vis with each glyph for a feature
- circle glyph splits to 4 parts, each for an attribute selection algorithm; and each slice further splits to multiple validation groups; each slice shows the importance of the feature
- glyphs sorted by overall importance but can switch to a scatter plot layout to explore two  metrics (average rank, most used) at the same time
- classifier view: compares the score produced by a combination of att. selection and classifier in tabular form
- select custom attributes to build own model"
