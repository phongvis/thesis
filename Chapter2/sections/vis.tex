...
\begin{figure}[!htb]
\centering
\subcaptionbox{Performance curve of the classification model with horizontal axis showing the false positive rate and vertical axis showing the false negative rate. rates.\label{fig:lr-Migut1}}[\columnwidth]{\includegraphics[width=\columnwidth]{clas-Migut1}} 
\\
...
\subsection{Summary}
\add{Visualization helps people carry out tasks more effectively by amplifying human cognition through visual representation and interaction. Visual analytics adds automated techniques to help analyze complex datasets and facilitate analytic reasoning. This thesis contributes novel visualization techniques and systems to support sensemaking. The designs are implemented and evaluated to validate their usefulness. The visualizations use provenance data captured during the sensemaking process. Next, we will discuss the literature on capture and visualization of provenance data.}