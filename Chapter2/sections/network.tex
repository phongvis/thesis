...
\subsection{Summary}
\add{This section reviews the most common techniques to visualize network data including node-link diagrams, matrix views and space-filling. \autoref{chap:sensemap} uses network data showing relationship between provenance data items such as opening relationship between web pages. Even though it uses an existing compact tree layout, the research discussed in this section provides a valuable knowledge for making suitable design decision. To address scalability, our tree visualization applies interactive features such as semantic zooming (appropriate designs for different levels of detail).}