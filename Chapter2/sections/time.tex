...
\begin{figure}[!htb]
	\centering
	\includegraphics[width=.7\columnwidth]{biography-chart}
	\caption[Joseph Priestley's Chart of Biography]{Joseph Priestley's Chart of Biography portraying the lifespans of famous persons. Each line segment represents a person, starting at when he/she was born and ending at when he/she died. \is{Priestley1765}}
...
\caption[Three charts showing the same time-series dataset]{Three charts showing the same time-series dataset with the horizontal axis representing time and the vertical axis representing a numerical value. \is{Aigner2011}}
...
\caption[Horizon Graph]{Horizon Graph: a space-efficient visualization of time-series data compared to line chart. \is{Reijner2008}}
...
Stacking multiple area charts on top of each other is a suitable approach to visualize multiple time-dependent variables. ThemeRiver~\cite{Havre2002} can be considered as a smooth and symmetric version of stacked graph, designed to show thematic variations over time within a large collection of documents (\autoref{fig:lr-theme-river}). Each theme is displayed as a colored current flowing through the time, and at any point, the width of the current maps to the strength of the associated theme. The overall river consists of multiple colored currents, providing a good overview of the themes that were important at certain points in time. 

\begin{figure}[!htb]
	\centering
	\includegraphics[width=.7\columnwidth]{theme-river}
	\caption[ThemeRiver]{ThemeRiver uses a river metaphor to represent themes with each colored current corresponding to a theme. The data is a collection of Fidel Castro's speeches, interviews and articles from the end of 1959 to mid-1961. \is{Havre2002}}
...
\begin{figure}[!htb]
\centering
\subcaptionbox{Traditional stacked graph with the baseline at 0. \label{fig:lr-streamgraph-1}}{\includegraphics[width=.7\columnwidth]{streamgraph-1}}

\vspace{0.5cm}

\subcaptionbox{ThemeRiver aiming for a symmetric layout around the horizontal axis. \label{fig:lr-streamgraph-2}}[\columnwidth]{\includegraphics[width=.7\columnwidth]{streamgraph-2}}

\vspace{0.5cm}

\subcaptionbox{Stream Graph minimizing the number of wiggles of layers. \label{fig:lr-streamgraph-3}}{\includegraphics[width=.7\columnwidth]{streamgraph-3}}
\caption[Stacked graphs with different design considerations]{Stacked graphs with different design considerations. \is{Aigner2011}}
...
\begin{figure}[!htb]
	\centering
	\includegraphics[width=.7\columnwidth]{lifelines}
	\caption[LifeLines]{LifeLines. The timeline consists of several facets, stacked vertically. Each facet includes health-related records shown as horizontal bars, which may be located in different rows to avoid overlapping. \is{Plaisant1998}}
...
\begin{figure}[!htb]
	...
	\caption[Storyline visualization of the movie \emph{Star Wars}]{Storyline visualization of the movie \emph{Star Wars}. Each line represents a character, and a bundle of lines represents an interaction of those characters. \is{Tanahashi2012}}
...
\begin{figure}[!htb]
	\centering
	\includegraphics[width=.7\columnwidth]{timenets}
	\caption[TimeNet visualization of genealogical data]{TimeNet visualization of genealogical data. Lifelines represent people with converging lines signifying marriage, and dotted lines indicating children. \is{Kim2010}}
...
\begin{figure}[!htb]
	...
	\caption[Spiral graph]{Spiral graph. Time is represented along a spiral with color intensity and line thickness are used to indicate time-dependent, numerical values. Two variables are distinguished using different color hue: yellow and red. \is{Weber2001}}
	\label{fig:lr-spiral}
\end{figure}
...
\hfill
\subcaptionbox{Spiral graph: no pattern is found with 24-day cycle. \label{fig:lr-spiral-2}}{\includegraphics[height=.325\columnwidth]{spiral-2}}
\hfill
\subcaptionbox{Spiral graph: weekly pattern is clear with 28-day cycle. \label{fig:lr-spiral-3}}{\includegraphics[height=.325\columnwidth]{spiral-3}}
\caption[Cycles in spiral graphs]{Different insights can be gained from visual representations depending on whether the linear or cyclic character of the data is emphasized. \is{Aigner2011}}
...
\begin{figure}[!htb]
\centering
\subcaptionbox{Cross section of a Douglas Fir tree showing its tree-rings. \is{Theron2006} \label{fig:lr-circle-view-1}}[.47\columnwidth]{\includegraphics[height=.3\columnwidth]{tree-ring}}
\hfill
\subcaptionbox{Circle View showing six variables over ten time steps. \is{Keim2004} \label{fig:lr-circle-view-2}}[.47\columnwidth]{\includegraphics[height=.3\columnwidth]{circle-view}}
\caption[Tree ring]{Tree ring. Concentric rings expanding from the center, each indicating a time step.}
...
\begin{figure}[!htb]
\centering
\subcaptionbox{Hierarchy is shown as a tree with temporal values annotated next to the nodes. \label{fig:lr-tree-ring-time}}[.48\columnwidth]{\includegraphics[height=.3\columnwidth]{tree-ring-time}}
\hfill
\subcaptionbox{Hierarchy is shown as a tree embedded on a circle view with rings indicating temporal values. \label{fig:lr-tree-ring-tree}}[.48\columnwidth]{\includegraphics[height=.3\columnwidth]{tree-ring-tree}}
\caption[Tree rings encoding both temporal and hierarchical information]{Tree rings encoding both temporal and hierarchical information at the same time. \is{Theron2006}}
\end{figure}
...
\begin{figure}[!htb]
	\centering
	\includegraphics[width=.7\columnwidth]{calendar}
	\caption[Calendar visualization]{Calendar visualization of a one-year data of daily power consumption. A cell for each day is color coded to reveal data patterns. \is{VanWijk1999}}
...
\begin{figure}[!htb]
	\centering
	\includegraphics[width=.7\columnwidth]{small-multiples}
	\caption[Small multiples of bar charts]{Small multiples of bar charts showing average annual Medicare spending on ambulance services per dialysis patient by U.S. state from 2001 to 2011. \is{SmallMultiples2014}}
...
Animation is another technique to convey time that can be applied to virtually all static visualization techniques. It relies on human perception in perceiving changes while a visualization smoothly updates from one time step to another. A notable example is Trendalyzer by Gapminder Foundation~\cite{Gapminder} -- an interactive visualization and presentation tool based on scatter plots (\autoref{fig:lr-animation}). The animation is controlled via a time slider, a play/pause button, and a speed slider. Only a few data items should be animated, and they are often highlighted so that the user can keep track of the changes. Trails may also be displayed to maintain the path of a data item through time. A study by Robertson~et~al.~\cite{Robertson2008} shows that animation is both slower and less accurate than small multiples in conveying trends over time.

\begin{figure}[!htb]
	\centering
	\includegraphics[width=.7\columnwidth]{animation}
	\caption[Trendalyzer]{Trendalyzer interface. A scatter plot with an animation controller to traverse through time. Additionally, trails are activated for the selected countries, Austria and Germany, which help to preserve the path of a variable in animation. \is{Aigner2011}}
