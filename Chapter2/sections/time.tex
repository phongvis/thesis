...
\subsection{Summary}
\add{This section reviews visualization of time-oriented data according to how time is visually encoded. Time can be mapped to a horizontal axis and a spiral axis. It can also be shown using a tree-ring and a calendar metaphor. Alternatively, small multiples and animation are additional options to indicate time. \autoref{chap:schemaline} contributes a novel timeline visualization technique that also supports sensemaking activities. Also, \autoref{chap:timesets} contributes a technique that can show both temporal and categorical information simultaneously.}