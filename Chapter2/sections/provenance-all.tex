...
%~\cite{Papazoglou2007}, ~\cite{Buyya2009}
...
\subsection{Summary}
\add{This section mainly reviews research on analytic provenance based on the four semantic-level model by Gotz and Zhou~\cite{Gotz2009}. Capture and visualization techniques for all levels -- event, action, sub-task and task -- are discussed. This thesis proposes an approach to automatically capture certain action types of the \emph{action} level and supports users to gain understanding of higher levels through visualization. It contributes novel visualization techniques of the captured provenance data to explore temporal (\autoref{chap:schemaline} and \autoref{chap:timesets}) and rational (\autoref{chap:sensepath} and \autoref{chap:sensemap}) relationship of sensemaking.} Literature of temporal and network visualization (to explore rational relationship) will be discussed in the next two sections, respectively.